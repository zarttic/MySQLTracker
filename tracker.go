...
func main() {
	...
	if *flagHelp || *flagUser == "" {
		fmt.Println("Usage: MySQL Tracker [options]")
		flag.PrintDefaults()
		return
	}
	...
	defer func() {
		if err := cleanGeneraLog(); err != nil {
			log.Printf("cleanGeneraLog error: %s \n", err)
		}
	...
	fmt.Println("start mysql tracker ...")
